...
$$logit(p) = log(\frac{p}{1-p}) = \beta_0 + \beta_1X_{scale\_median\_inc} + \beta_2X_{scale\_age} + \beta_3I_{RegionQuebec}$$ 
where

- $\beta_0$ = 0.04, $\beta_1$ = -0.23, $\beta_2$ = 0.38, and $\beta_3$ = 0.32;
- $p$ is the estimated probability that individual would buy Active/Advance product;
- $X_{scale\_median\_inc}$ represents the scaled median income variable from 41880~195570 to 0~1;
- $X_{scale\_age}$ represents the scaled age variable, which scales the age from 18~92 to 0~1;
- $I_{RegionQuebec}$ represents the region variable where Quebec is 1 and reference Alberta is 0.
...
inf_table_1 <- rbind(inf_table_1[1:3,], inf_table_1[11,])
rownames(inf_table_1) <- c("Baseline odds ratio", "Scaeled median income", "Scaled age", "Quebec")
